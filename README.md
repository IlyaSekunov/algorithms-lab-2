...
![image](charts/512-rectangles-512^2-points.png)

При количестве точек меньше чем $N^2$, то предподоготовка матрицы $N^3$ будет сильно влиять на итоговый результат, поэтому видно, что матрица работает сильно хуже брут форса и дерева отрезков.

![image](charts/128-rectangles-128^3-points.png)

При количестве точек более чем или равно $N^3$ матрица при поиске работает даже быстрее чем дерево, если учитывать константы. В таком случает предподоготовка занимает небольшую часть суммы работы алгоритма и из-за количества точек равное $N^3$ множитель $M \cdot \log N$ делает больший вклад в работу алгоритма и матрица начинает вырываться вперед. 
